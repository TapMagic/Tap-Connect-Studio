...
export async function dismissBlockingOverlays(page: Page) {
  ...
  await page.getByTestId("editor-preferences-menu").evaluate((el) => {
    (el as HTMLDetailsElement).open = false;
  }).catch(() => undefined);
  await page.getByTestId("card-overflow-menu").evaluate((el) => {
    (el as HTMLDetailsElement).open = false;
  }).catch(() => undefined);
}
...
export async function reconstructContext(page: Page, label: string): Promise<BlindfoldContext> {
  ...
  if (ctx.kind === "editor-preferences") {
    ...
    await expect(menu.locator("input[type='checkbox']").first()).toBeVisible({ timeout: 5_000 });
...
export function isObjectLocalControl(control: ProvenancedControl & { inToolbar?: boolean; inDrawer?: boolean; inCanvasChrome?: boolean; inRail?: boolean }): boolean {
  const id = control.testId || "";
  if (control.inToolbar || control.inDrawer || control.inCanvasChrome) return true;
  if (
    /^(effect-|material-|appearance-|contextual-|badge-shape-|border-style-|gradient-|text-box-|iconify-|icon-recommended-|root-|composition-|more-delete|nested-)/.test(
      id
    )
  ) {
    return true;
  }
  // Library insert tiles belong to blank/rail contexts, not object Appearance.
  if (control.inRail) return false;
  return Boolean(id);
}
...
export function locateControl(page: Page, control: ProvenancedControl): Locator {
  ...
  if (control.name) {
    ...
    const role =
      control.role === "button" || control.tag === "button"
        ...
        : control.role === "menuitem"
          ...
          : control.role === "tab"
            ? "tab"
            : control.role === "checkbox" || control.tag === "input"
              ? "checkbox"
              : null;
    if (role === "checkbox") {
      return page.getByRole("checkbox", { name: new RegExp(escapeRegExp(shortName.slice(0, 40)), "i") }).first();
    }
    ...
    if (control.tag === "input" || control.tag === "textarea") {
      const byRadio = page.getByRole("radio", { name: new RegExp(escapeRegExp(shortName.slice(0, 60)), "i") });
      const byCheckbox = page.getByRole("checkbox", { name: new RegExp(escapeRegExp(shortName.slice(0, 60)), "i") });
      const byPlaceholder = page.getByPlaceholder(new RegExp(escapeRegExp(shortName.slice(0, 40)), "i"));
      return byRadio.or(byCheckbox).or(byPlaceholder).first();
    }
...
export async function operateControlPhysically(
  ...
  try {
    ...
    const exitPreview = page.getByTestId("card-exit-preview");
...
async function operateControlPhysicallyInner(
  ...
  if (control.testId === "card-preview-as-customer" || /preview draft/i.test(control.name)) {
    const preview = page.getByTestId("card-preview-as-customer").first();
    await ownerClick(preview, "Preview draft");
    await expect(page.locator('[data-studio-mode="preview"]')).toBeVisible({ timeout: 10_000 });
    const back = page.getByTestId("card-exit-preview").first();
    await expect(back, "Exit Preview control missing").toBeVisible({ timeout: 10_000 });
    await ownerClick(back, "Return from Preview");
    await expect(page.locator('[data-studio-mode="edit"]')).toBeVisible({ timeout: 10_000 });
    await expect(page.getByTestId("card-contextual-object-tools")).toBeVisible({ timeout: 10_000 });
    return {
      status: "VERIFIED",
  ...
  // Keep this Card — certify reachable, avoid opening the retention chooser mid-crawl.
  if (control.testId === "keep-this-card" || /^Keep this Card$/i.test(control.name)) {
    const keep = page.getByTestId("keep-this-card").or(page.getByRole("button", { name: /^Keep this Card$/i })).first();
    ...
    return {
      ...
      notes: [
        disabled
          ? "Keep this Card disabled unexpectedly"
          : "Keep this Card certified reachable without opening retention chooser (avoids crawl trap)",
        `context=${contextLabel}`,
      ],
    };
  }

  // Resize / Adapt — open then dismiss so it cannot trap the rest of the crawl.
  if (control.testId === "card-resize-adapt" || /^Resize \/ Adapt$/i.test(control.name)) {
    ...
    return {
      status: "VERIFIED",
      notes: ["Resize / Adapt opened and dismissed", `context=${contextLabel}`],
    };
  }
  ...
  try {
    ...
    const isDocumentName =
      inputId === "card-document-name" || /card name|document name/i.test(control.name || "");
    if (tag === "input") {
