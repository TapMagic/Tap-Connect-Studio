...
test.describe("selection, nested content, and appearance parity", () => {
  ...
  test("parent/child modes, reflow, background opacity, and routing", async ({ page }) => {
    ...
    const children = root.locator('[data-composition-node][data-element-kind]');
    ...
    await page.getByRole("button", { name: /Edit children on canvas|Finish editing children/ }).click();
    ...
    await expect(root.locator('[data-composition-node][data-selected="true"] [data-component-kind="container"]')).toHaveCount(1);
    ...
    await page.getByTestId("root-background-opacity").fill("25");
    ...
    for (let i = 0; i < 5; i += 1) {
      await page.getByTestId("card-preview-draft").click().catch(async () => {
        await page.getByRole("button", { name: /Preview draft|Preview/ }).first().click();
      });
      await page.waitForTimeout(200);
      await page.getByRole("button", { name: /Exit preview|Finish editing|Back/ }).first().click().catch(() => undefined);
    }
