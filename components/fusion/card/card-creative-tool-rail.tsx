...
function IconLibraryDrawer({ add, matches, targetChoice }: { model: CardEditorLiveModel; add: (kind: CardElementKind, props?: Record<string, unknown>) => void; matches: (value: string) => boolean; targetChoice: ReactNode }) {
  ...
  useEffect(() => {
    if (query.trim().length < 2) { setProviderIcons([]); setStatus("idle"); return; }
    ...
    const timer = window.setTimeout(() => {
      void fetch(`/api/creative/icons?q=${encodeURIComponent(query)}`, { signal: controller.signal })
        ...
        .catch(() => { setProviderIcons([]); setStatus("fallback"); });
    }, 220);
    return () => { window.clearTimeout(timer); controller.abort(); };
  }, [query]);
  const place = (props: Record<string, unknown>) => add("icon", props);
  return <div className="space-y-3" data-testid="card-icon-library">{targetChoice}<p className="text-[10px] text-white/55">Choosing an Icon creates a normal TapConnect Icon Element. Quick Add Sparkles lives under Tools.</p><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search Lucide · Tabler · Phosphor · Remix · Material Symbols" className="h-10 w-full rounded border border-white/15 bg-black/20 px-3 text-xs" data-testid="icon-library-search" /><div className="flex flex-wrap gap-1">{["star", "heart", "map", "ticket", "gift", "phone", "mail", "sparkles"].map((term) => <button key={term} type="button" className="min-h-8 rounded border border-white/10 px-2 text-[10px]" onClick={() => setQuery(term)}>{term}</button>)}</div><p className="text-[9px] text-white/40" data-testid="icon-library-status">{status === "loading" ? "Searching Iconify…" : status === "fallback" ? "Provider unavailable — showing built-in fallback icons" : status === "ready" ? `${providerIcons.length} Iconify results` : "Type at least 2 characters to search"}</p><h3 className="text-[10px] font-semibold uppercase text-white/45">TapConnect Recommended</h3><div className="grid grid-cols-2 gap-1" data-testid="icon-library-recommended">{ICON_LIBRARY.filter((icon) => matches(`${icon.label} ${icon.category}`) && (!query.trim() || `${icon.label} ${icon.category}`.toLowerCase().includes(query.toLowerCase()))).map((icon) => <button key={icon.id} type="button" className="min-h-12 rounded border border-white/10 px-2 text-left text-[10px]" onClick={() => place({ icon: icon.id, iconProvider: "native", iconCollection: "lucide", iconName: icon.id, accessibleLabel: icon.label, decorative: false })}>✦ {icon.label}<span className="block text-[8px] text-white/40">{icon.category}</span></button>)}</div><h3 className="text-[10px] font-semibold uppercase text-white/45">Iconify results</h3><div className="grid grid-cols-2 gap-1" data-testid="icon-library-iconify-results">{providerIcons.map((icon) => <button key={icon.canonicalId} type="button" className="min-h-12 rounded border border-white/10 px-2 text-left text-[10px]" onClick={() => place({ icon: icon.canonicalId, iconProvider: "iconify", iconCollection: icon.collection, iconName: icon.name, iconSource: icon.source, accessibleLabel: icon.name, decorative: false })}>{icon.name}<span className="block text-[8px] text-white/40">{icon.collection}</span></button>)}</div></div>;
}
...
function ButtonLibrary({ add, matches }: { add: (kind: CardElementKind, props?: Record<string, unknown>) => void; matches: (value: string) => boolean }) {
  return <div className="space-y-3" data-testid="card-button-library"><p className="text-[10px] text-white/55">Starter Buttons — structurally varied Surfaces. Every tile creates the same canonical Button composition.</p><div className="grid grid-cols-2 gap-2">{STARTER_BUTTON_PRESETS.filter((preset) => matches(preset.label)).map((preset) => <button key={preset.id} type="button" className="group min-h-24 rounded-lg border border-white/10 p-2 text-left hover:border-[#b8ff2c]/50" data-testid={`button-preset-${preset.id}`} onClick={() => add("button", { ...preset.props, accessibleLabel: String(preset.props.label || preset.label), trackingName: `button-${preset.id}` })}><span className="flex min-h-12 items-center justify-center border border-white/10 px-2 text-center text-[10px] font-semibold" style={{ borderRadius: Number(preset.props.radius || 0), background: String("gradientStart" in preset.props ? `linear-gradient(120deg,${preset.props.gradientStart},${preset.props.gradientEnd})` : preset.props.fill || "#334155"), color: String(preset.props.labelColor || "#fff"), borderWidth: Number(preset.props.borderWidth || 0), borderColor: String(preset.props.borderColor || "transparent") }}>{String(preset.props.label || preset.label)}</span><span className="mt-2 block text-[10px] text-white/75">{preset.label}</span></button>)}</div></div>;
}
