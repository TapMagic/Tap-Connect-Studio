...
import {
  ...
  STARTER_HERO_PRESETS,
  STARTER_LAUNCH_PRESETS,
  STARTER_BOTTOM_STOP_PRESETS,
...
import { CURATED_FAMILY_MISSION_CONTROL_ID } from "@/lib/fusion/creative-studio/visual-parts";
...
function QuickToolsDrawer({ model, add }: { model: CardEditorLiveModel; add: (kind: CardElementKind, props?: Record<string, unknown>) => void }) {
  return <div className="space-y-2" data-testid="card-quick-tools"><p className="text-[10px] text-white/55">Compact commands use the same placement and mutation authority as the libraries.</p><div className="grid grid-cols-2 gap-2"><LibraryAction label="Select" description="Return to selection" onClick={() => { model.setSelectedId(null); model.setSelectedCompositionNodeIds?.([]); }} /><LibraryAction label="Text" description="Place editable text" onClick={() => add("text", { text: "Type here" })} /><LibraryAction label="Quick Add Icon" description="Place default Sparkles Icon" onClick={() => add("icon", { icon: "sparkles", accessibleLabel: "Decorative icon", decorative: true })} /><LibraryAction label="Shape" description="Place a shape" onClick={() => add("decorative_graphic", { shape: "rectangle" })} /><LibraryAction label="QR" description="Place managed QR artwork" onClick={() => add("qr_image", { qrManagementState: "setup_required", accessibleLabel: "QR code setup required" })} /></div><div className="space-y-2 pt-2" data-testid="starter-component-quick-add"><h3 className="text-[10px] font-semibold uppercase text-white/45">Starter Forms</h3>{STARTER_FORM_PRESETS.map((preset) => <button key={preset.id} type="button" className="min-h-10 w-full rounded border border-white/10 text-xs" data-testid={`starter-form-${preset.id}`} onClick={() => add("form", { ...preset.props, componentKind: "form", liveSubmission: false })}>{preset.label}</button>)}<h3 className="text-[10px] font-semibold uppercase text-white/45">Starter Containers</h3>{STARTER_CONTAINER_PRESETS.map((preset) => <button key={preset.id} type="button" className="min-h-10 w-full rounded border border-white/10 text-xs" data-testid={`starter-container-${preset.id}`} onClick={() => add("composition", { ...preset.props, componentKind: "container", elementKind: "composition" })}>{preset.label}</button>)}<h3 className="text-[10px] font-semibold uppercase text-white/45">Starter Dividers</h3>{STARTER_DIVIDER_PRESETS.map((preset) => <button key={preset.id} type="button" className="min-h-10 w-full rounded border border-white/10 text-xs" data-testid={`starter-divider-${preset.id}`} onClick={() => add("divider", { ...preset.props })}>{preset.label}</button>)}<h3 className="text-[10px] font-semibold uppercase text-white/45">Hero Sections</h3>{STARTER_HERO_PRESETS.map((preset) => <button key={preset.id} type="button" className="min-h-10 w-full rounded border border-white/10 text-xs" data-testid={`starter-hero-${preset.id}`} onClick={() => add("composition", { ...preset.props, componentKind: "container", elementKind: "composition" })}>{preset.label}</button>)}<h3 className="text-[10px] font-semibold uppercase text-white/45">Launch Blocks</h3>{STARTER_LAUNCH_PRESETS.map((preset) => <button key={preset.id} type="button" className="min-h-10 w-full rounded border border-white/10 text-xs" data-testid={`starter-launch-${preset.id}`} onClick={() => add("button", { ...preset.props })}>{preset.label}</button>)}<h3 className="text-[10px] font-semibold uppercase text-white/45">Bottom Stops</h3>{STARTER_BOTTOM_STOP_PRESETS.map((preset) => <button key={preset.id} type="button" className="min-h-10 w-full rounded border border-white/10 text-xs" data-testid={`starter-bottom-stop-${preset.id}`} onClick={() => add("composition", { ...preset.props, componentKind: "container", elementKind: "composition" })}>{preset.label}</button>)}<h3 className="text-[10px] font-semibold uppercase text-white/45">Action Groups</h3><button type="button" className="min-h-10 w-full rounded border border-white/10 text-xs" data-testid="starter-action-stack-rails" onClick={() => model.onInsertActionGroup?.("one_column", [{ label: "Call", icon: "phone", iconSecondary: "arrow-up-right" }, { label: "View Our Current Listings", icon: "sparkles", iconSecondary: "arrow-up-right" }, { label: "Book an Appointment Today", icon: "calendar", iconSecondary: "arrow-up-right" }, { label: "Directions", icon: "map-pin", iconSecondary: "arrow-up-right" }], "Added rail Action stack")}>Rail Action Stack</button><button type="button" className="min-h-10 w-full rounded border border-white/10 text-xs" data-testid="starter-action-grid-two" onClick={() => model.onInsertActionGroup?.("two_column", [{ label: "Call", icon: "phone" }, { label: "Listings", icon: "sparkles" }, { label: "Book Today", icon: "calendar" }, { label: "Directions", icon: "map-pin" }], "Added two-column Action grid")}>Two-Column Action Grid</button><button type="button" className="min-h-10 w-full rounded border border-white/10 text-xs" data-testid="starter-round-team-grid" onClick={() => model.onInsertActionGroup?.("round_team_grid", [{ label: "Alex", portrait: true, iconMediaUrl: "/tap-connect-mark.png" }, { label: "Jordan", portrait: true, iconMediaUrl: "/tap-connect-logo.png" }, { label: "Sam", portrait: true, iconMediaUrl: "/tap-connect-mark.png" }, { label: "Riley", portrait: true, iconMediaUrl: "/tap-connect-logo.png" }], "Added round team grid")}>Round Team Grid</button><button type="button" className="min-h-10 w-full rounded border border-white/10 text-xs" data-testid="starter-industrial-action" onClick={() => model.onInsertActionGroup?.("one_column", [{ label: "Mission Control", icon: "sparkles", familyId: CURATED_FAMILY_MISSION_CONTROL_ID, actionType: "website", href: "https://host.example/mission" }], "Added Mission Control Action")}>Mission Control Action</button></div><div className="space-y-1 pt-2"><button type="button" disabled className="min-h-10 w-full rounded border border-white/10 text-xs opacity-50">Charts · data contract required</button><button type="button" disabled className="min-h-10 w-full rounded border border-white/10 text-xs opacity-50">Captions · media required</button><button type="button" disabled={!model.mediaUploadReady} className="min-h-10 w-full rounded border border-white/10 text-xs opacity-50">Background removal · {model.mediaUploadReady ? "select an image" : "provider setup required"}</button></div></div>;
}
