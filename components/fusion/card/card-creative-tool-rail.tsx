...
function TextLibrary({ model, add, matches, targetChoice, targetSectionId }: { model: CardEditorLiveModel; add: (kind: CardElementKind, props?: Record<string, unknown>) => void; matches: (value: string) => boolean; targetChoice: ReactNode; targetSectionId: string | null }) {
  ...
  const insertCombination = (combination: (typeof STARTER_TEXT_COMBINATIONS)[number]) => {
    model.onAddObjects?.(combination.lines.map((line, index) => ({ kind: "text", initialProps: { text: line.text, fontSize: line.fontSize, fontWeight: line.fontWeight, fontFamily: line.fontFamily, letterSpacingEm: line.letterSpacingEm, textAlign: "center", gradientFill: combination.style, ...(combination.effects || {}), combinationId: combination.id, combinationOrder: index, yNudge: line.yOffset } })), targetSectionId, `Added ${combination.label} Text combination`);
  };
